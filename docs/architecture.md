...
**The OAuth client itself is shared, not per-brand** (the standard SaaS shape: one app-level client, a separate token per connected account) - `GMAIL_CLIENT_ID`/`GMAIL_CLIENT_SECRET` (env vars, treated as a "Web application" client, and taking precedence) or else the JSON file at `GMAIL_CREDENTIALS_PATH` (`integrations/gmail/oauth.py::gmail_client_config`) is one Google Cloud OAuth client used for every brand's consent flow (only the resulting per-brand *token* differs). The existing client (used since Phase 1) is an "installed" (Desktop app) type, whose registered `redirect_uris` field Google actually ignores in favor of its own loopback-IP rule - any `http://localhost:<port>/<path>` redirect is accepted regardless of what's literally in the client JSON, confirmed live: the real Google consent screen came back correctly (no `redirect_uri_mismatch`) for `http://localhost:8000/brand-kit/gmail/callback`. This means local dev works out of the box with the existing client, but a real deployment on an actual domain (not `localhost`) will need a **"Web application"**-type OAuth client in Google Cloud Console with that domain's `/brand-kit/gmail/callback` registered as an authorized redirect URI - For a deployment, setting `GMAIL_CLIENT_ID`/`GMAIL_CLIENT_SECRET` to that Web client is the only change needed (no file to mount); a file at `GMAIL_CREDENTIALS_PATH` still works, and `build_authorization_flow` auto-detects "web" vs "installed" from it. The image runs uvicorn with `--proxy-headers --forwarded-allow-ips='*'` so that behind a TLS-terminating proxy the callback URL is built as `https://` - without it the redirect_uri sent to Google is `http://` and fails with `redirect_uri_mismatch`. `docker-compose.yml` mounts `./secrets:/srv/secrets:ro` into the app container specifically so this in-app flow (unlike the old script, which always ran on the host) can read that file at runtime.
...
## Draft from an idea (no fetch or research step)

Every other source (Gmail, GitHub, RSS, WooCommerce/Shopify) needs an LLM call to first decide *which* fetched items are worth posting about - `routes_board.py`'s `idea_to_angle` skips that entirely: the user's own free-text idea already is the angle, no extraction needed.

- Always shown on the board (unlike GitHub/Website, which only appear once a brand has repos/a catalog configured) - "Draft from an idea..." opens a modal with just a required idea textarea, optional extra context, and the same optional format/language override `draft_from_website` has.
- `idea_to_angle(idea, extra_context)` builds the one angle dict directly - `title` is the idea's first line (capped at 120 chars), `summary` is the idea verbatim, `full_text` folds `extra_context` in (there's no angle-extraction prompt to hand it to instead), always `suitable_for_social=True` with `priority_score=100` - the user explicitly asked for this one.
- `_run_idea_draft` hands that single angle straight to the same `create_content_items` every other source uses, backgrounded the same way as `draft_github_posts`/`draft_from_website` (the graph run's Analytical-node LLM call is the slow part, same reasoning).
...
Set `GMAIL_CLIENT_ID`/`GMAIL_CLIENT_SECRET` in `.env` (a "Web application" client; register `http://localhost:8000/brand-kit/gmail/callback` too for local dev), or drop the downloaded client JSON in place:
